...
本项目设计一个CDN可访问性测试工具，通过解析HTML配置文件获取API参数，递归遍历栏目树形结构，提取并验证所有图片资源和APK下载链接的可访问性，输出包含完整页面路径的测试报告。采用模块化设计，分为配置解析、认证管理、节点遍历、资源提取、可访问性验证和报告生成六个核心模块，确保代码清晰、可维护、可扩展。
...
|------|----------|----------|----------|----------|
| 配置解析 | HTML文件 | 高 | 检查文件是否存在，提供默认配置 | 文件不存在时退出并提示 |
...
class ConfigParser:
    ...
    def parse_html_config(self, html_path: str) -> dict:
        """从HTML文件解析配置"""
...
|--------|----------|----------|----------|----------|
| HTML配置格式变更 | 高 | 低 | 中 | 使用健壮的正则表达式，提供默认配置 |
